...
const protocolHandler = new ProtocolHandler(buildConfig.protocols);
const desktopAgentManager = new DesktopAgentManager();
let desktopAgentSessionPollTimer: ReturnType<typeof setInterval> | null = null;
let desktopAgentSessionProbeInFlight = false;
let desktopAgentPowerSaveTimer: ReturnType<typeof setInterval> | null = null;

function stopDesktopAgentSessionPolling(): void {
    if (desktopAgentSessionPollTimer) {
        clearInterval(desktopAgentSessionPollTimer);
        desktopAgentSessionPollTimer = null;
    }
}

function requestDesktopAgentSession(): void {
    global.mainWindow?.webContents.send("requestDesktopAgentSession");
}

async function probeDesktopAgentSessionFromPage(): Promise<void> {
    if (!global.mainWindow || desktopAgentSessionProbeInFlight) {
        return;
    }

    desktopAgentSessionProbeInFlight = true;
    try {
        const payload = await global.mainWindow.webContents.executeJavaScript(
            `(() => {
                try {
                    ...
                    return { accessToken, homeserverUrl, userId };
                } catch {
                    return null;
                }
            })()`,
        ...
        if (payload && typeof payload === "object") {
            console.log(`[desktop-agent] page session probe succeeded: ${JSON.stringify(payload)}`);
            stopDesktopAgentSessionPolling();
            desktopAgentManager.updateSession(payload);
        }
    } catch (error) {
        console.warn("[desktop-agent] page session probe failed", error);
    } finally {
        desktopAgentSessionProbeInFlight = false;
    }
}

function startDesktopAgentSessionPolling(): void {
    stopDesktopAgentSessionPolling();
    requestDesktopAgentSession();
    void probeDesktopAgentSessionFromPage();
    desktopAgentSessionPollTimer = setInterval(() => {
        requestDesktopAgentSession();
        void probeDesktopAgentSessionFromPage();
    }, 3000);
}

function stopDesktopAgentPowerSavePolling(): void {
    if (desktopAgentPowerSaveTimer) {
        clearInterval(desktopAgentPowerSaveTimer);
        desktopAgentPowerSaveTimer = null;
    }
}

function startDesktopAgentPowerSavePolling(): void {
    stopDesktopAgentPowerSavePolling();
    void desktopAgentManager.reconcilePowerSaveBlocker();
    desktopAgentPowerSaveTimer = setInterval(() => {
        void desktopAgentManager.reconcilePowerSaveBlocker();
    }, 15000);
}
        ...
        try {
            ...
            if (e instanceof SyntaxError) {
                ...
                void dialog.showMessageBox({
                    type: "error",
                    title: `Your ${global.vectorConfig.brand || "Dopax"} is misconfigured`,
                    message:
                        `Your custom ${global.vectorConfig.brand || "Dopax"} configuration contains invalid JSON. ` +
                        `Please correct the problem and reopen ${global.vectorConfig.brand || "Dopax"}.`,
                    detail: e.message || "",
                });
            }
...
app.on("ready", async () => {
    ...
    try {
        ...
        await loadConfig();
        desktopAgentManager.updateConfig(global.vectorConfig);
        startDesktopAgentPowerSavePolling();
    } catch (e) {
    ...
    ipcMain.on("desktopAgentSession", (_event, payload) => {
        console.log(
            `[desktop-agent] ipc session payload received: ${
    ...
    ipcMain.on("desktopAgentSignedOut", () => {
        console.log("[desktop-agent] sign-out event received, stopping background service");
        void desktopAgentManager.stop();
        startDesktopAgentSessionPolling();
    });

    ipcMain.handle("getDesktopAgentStatus", async () => {
        return desktopAgentManager.getStatus();
    });

    ipcMain.handle("controlDesktopAgent", async (_event, action: unknown) => {
        if (action !== "start" && action !== "restart") {
            throw new Error(`Unsupported desktop agent control action: ${String(action)}`);
        }

        return desktopAgentManager.control(action);
    });
                ...
                dialog.showMessageBoxSync(global.mainWindow, {
                    ...
                    buttons: [
                        ...
                        _t("action|close_brand", {
                            brand: global.vectorConfig.brand || "Dopax",
                        }),
                    ],
